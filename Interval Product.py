...
class segtree(object):
  ...
  def __query_aux(self, i, ilow, ihi, low, hi):
    """Query the partial multiplication of the interval corresponding to the
...
def main():
	...
	while(len(entrada) != 0):
		len_secuencia, rondas = map(int, entrada.split())
		secuencia = list(map(int,stdin.readline().strip().split()))
		tree = segtree(secuencia)
		cad = ""
		for i in range(rondas):
			comando = stdin.readline().strip().split()
			if(comando[0] == 'C'):
				#en la posición [comando[1] - 1 ] del arbol cambia el valor de a comando[2]
				tree.update(int(comando[1]) - 1 ,int(comando[2]))
			else:
				ans = tree.query(int(comando[1]) - 1,int(comando[2]))
				if(ans == 0):
					cad += "0"
				elif(ans < 0):
					cad += "-"
					...
					cad += "+"
				#multiplica desde [comando[1] -1 ] hasta [comando[2] - 1]
		print(cad)
		entrada = stdin.readline().strip();
